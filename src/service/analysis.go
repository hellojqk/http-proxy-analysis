...
func Analysis() {
	...
	var count int64
	repository.DB.Model(&entity.ProxyLog{}).Where("status=1 and proxy_response_body is not null and proxy_response_body <> '' and image_response_body is not null and image_response_body <> '' and analysis_status = 'N'").Count(&count)
	fmt.Printf("Analysis总计:%d\n", count)

	for (pageIndex-1)*pageSize < int(count) {
		result := make([]entity.ProxyLog, 0, 10)
		err := repository.DB.Preload("Application").Where("status=1 and proxy_response_body is not null and proxy_response_body <> '' and image_response_body is not null and image_response_body <> ''and analysis_status = 'N'").Order("id asc").Limit(pageSize).Offset((pageIndex - 1) * pageSize).Find(&result).Error
		if err != nil {
			log.Err(err).Msg("DB.Raw")
		}
		for _, proxyLog := range result {
			var left = proxyLog.ProxyResponseBody
			var right = proxyLog.ImageResponseBody
			if proxyLog.Application.Main == "image" {
				left = proxyLog.ImageResponseBody
				right = proxyLog.ProxyResponseBody
			}
			diffResult, err := jsondiff.Diff(left, right, true)
			if err != nil {
			...
			repository.DB.Model(&entity.ProxyLog{}).Where(&entity.ProxyLog{Model: entity.Model{ID: proxyLog.ID}}).UpdateColumns(map[string]interface{}{"analysis_status": "Y", "analysis_result": string(diffResultBts), "analysis_diff_count": len(saveResult)})
		}
		pageIndex++
	}
}
